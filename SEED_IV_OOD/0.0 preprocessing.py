...
def save_datas_seg(window, stride, data_dir, saved_dir):
    ...
    for subidx in range(0,15):
        ...
        np.savez(join(saved_dir, str(subnums[subidx])), x=x, y=y) 
...
saved_dir = join(DATAS, 'SEED_npz')

# There are 2 methods
# save_datas_seg(WINDOW, STRIDE, DATA,join(saved_dir, 'seg_raw'))
...
# load data
saved_dir = join(DATAS, 'SEED_npz', 'seg_DE')
