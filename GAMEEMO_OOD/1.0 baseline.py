...
train_path = Path(join(os.getcwd(), 'results', project_name, train_name))
...
def run_train():
    ...
    scheduler = lr_scheduler.CosineAnnealingWarmUpRestarts(optimizer,T_0=STEPS,T_mult=1,eta_max=1e-3,T_up=STEP*3,gamma=0.5)
...
train_path = Path(join(os.getcwd(), 'results', project_name, 'valence'))
