...
PROJECT = f'Base_pvalue2_{int(THRESHOLD*100)}'
...
def set_remove_ood(datas, targets, train_x, train_y, valid_x, valid_y):
    ...
    ood_detector_path = Path(join(os.getcwd(), 'results', DATASET_NAME, MODEL_FEATURE, 'Base_Remove0', train_name))
    ...
    with torch.no_grad():
        for (x, y, subID) in loader:
            for i in range(len(x)):
                ...
                dx = dx.to(device)
                xs = ood_detector.get_features(dx)
                p_value = pvalue_ood(y[i], xs)
                pvalues.append(p_value)
    pvalues = np.concatenate(tuple(pvalues))

    presult = np.array(pvalues)
